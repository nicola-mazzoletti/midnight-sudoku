...
const promptSolutionGrid = async (rli: Interface): Promise<bigint[][]> => {
  console.log('Enter the solution row by row (space-separated values 1–4, e.g. "1 2 3 4"):');
  const solution: bigint[][] = [];
  for (let row = 0; row < 4; row++) {
    const line = await rli.question(`Row ${row + 1}: `);
    solution.push(line.trim().split(/\s+/).map((v) => BigInt(v)));
  }
  return solution;
};
 ...
 *  +-------+-------+
 */
const displayPuzzleGrid = (grid: bigint[][], label: string, logger: Logger): void => {
  const divider = '+-------+-------+';
  ...
  for (let r = 0; r < 4; r++) {
    const row = grid[r];
    logger.info(`| ${cell(row[0])}   ${cell(row[1])} | ${cell(row[2])}   ${cell(row[3])} |`);
...
const deployOrJoin = async (
  ...
  while (true) {
    ...
    switch (choice) {
      ...
      case '2': {
        const rawAddress = await rli.question('What is the contract address (in hex)? ');
        ...
        if (existing) {
          ...
          logger.info('No existing private state found. Please enter your solution.');
          solution = await promptSolutionGrid(rli);
        }

        const api = await SudokuAPI.join(providers, rawAddress, solution, logger);
        logger.info('=====================================');
        logger.info(`Contract address: ${api.deployedContractAddress}`);
        logger.info('=====================================');
        return api;
 ...
 */
const displayDerivedState = (state: SudokuDerivedState | undefined, logger: Logger): void => {
  if (state === undefined) {
...
const mainLoop = async (
  ...
  const subscription = api.state$.subscribe({ next: (state) => (currentState = state) });
  ...
  const solverPk = Buffer.from(walletProvider.getCoinPublicKey().toString(), 'hex');
        ...
        switch (choice) {
          ...
          case '2': {
            if (!currentState?.solution || currentState.solution.length === 0) {
              logger.info('No solution stored yet. Please enter your solution first.');
...
export const run = async (config: Config, testEnv: TestEnvironment, logger: Logger): Promise<void> => {
  ...
  try {
    ...
    const zkConfigProvider = new NodeZkConfigProvider<SudokuCircuitKeys>(config.zkConfigPath);
    const providers: SudokuProviders = {
      privateStateProvider: levelPrivateStateProvider<PrivateStateId, SudokuPrivateState>({
